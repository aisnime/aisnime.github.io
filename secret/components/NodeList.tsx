...
import { Server, Smartphone, Monitor, Cpu, Wifi, Skull, RefreshCw, Zap, ShieldAlert, MapPin, Globe, Database, Coins, Clock, CreditCard, Activity, Download, Send } from 'lucide-react';
...
const NodeList: React.FC<Props> = ({ nodes, onRecoverNode, onTriggerAttack }) => {
  ...
  // Mock Client Data untuk tampilan detail di bawah
  const clientData = {
    ip: '192.168.1.105',
  ...
  return (
    <div className="bg-cyber-800 border border-cyber-700 rounded-lg h-full overflow-hidden flex flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto space-y-2 px-4 pb-2">
        {nodes.length === 0 && (
        ...
        {nodes.slice().reverse().map((node) => (
          ...
          >
            ...
            {/* COL 1: Identity (Icon + ID) - Removed Type from here */}
            ...
            {/* COL 2: Network Info (IP & Location & Device Type) - Added Type here */}
      ...
      <div className="border-t border-cyber-700 bg-cyber-900/80 p-4 shrink-0 backdrop-blur-sm">
         ...
         <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
             <div className="bg-black/30 p-2 rounded border border-cyber-700">
